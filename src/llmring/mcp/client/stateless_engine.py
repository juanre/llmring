...
class StatelessChatEngine:
    ...
    async def execute_tool(
        ...
        # Execute tool
        result = await mcp_client.execute_tool(tool_call.tool_name, tool_call.arguments)
    ...
    async def _create_context(self, request: ChatRequest) -> ProcessingContext:
        ...
        if request.conversation_id:
            ...
            conversation = await self.conversation_manager.load_conversation(
                request.conversation_id, request.auth_context
            )
        ...
        if response.tool_calls:
            ...
            for tc in response.tool_calls:
                ...
                if context.mcp_client:
                    try:
                        result = await context.mcp_client.execute_tool(
    ...
    def _prepare_messages(self, context: ProcessingContext) -> list[dict[str, Any]]:
