...
def _write_env_file(path: Path, server_url: str, api_key: str, overwrite: bool = False) -> None:
    ...
    lines = [
        ...
        f"LLMRING_SERVER_URL={server_url}",
        f"LLMRING_API_KEY={api_key}",
        "",
    ]
...
async def cmd_server_init(args):
    ...
    print()
    print(format_success(f"✅ Saved configuration to {env_file}"))
    ...
    print('  llmring chat "Hello" --model fast')
    return 0
...
async def _ensure_server_env(
    *,
    ...
    if not server_url:
        print("❌ Error: No llmring-server configured.")
        print("Set LLMRING_SERVER_URL or run `llmring server init` to create a local config.")
        return None

    if require_api_key and not api_key:
        print("❌ Error: LLMRING_API_KEY environment variable required for this command.")
        if used_saas:
