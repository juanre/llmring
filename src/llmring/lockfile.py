...
class AliasBinding(BaseModel):
    ...
    @classmethod
    def from_model_ref(cls, alias: str, model_ref: str, constraints: Optional[Dict] = None) -> "AliasBinding":
        """Create from a model reference string like 'openai:gpt-4'."""
        if ":" not in model_ref:
            raise ValueError(f"Invalid model reference: {model_ref}. Expected format: provider:model")
...
class ProfileConfig(BaseModel):
    ...
    registry_versions: Dict[str, int] = Field(
        ...
        description="Pinned registry versions per provider"
    )
