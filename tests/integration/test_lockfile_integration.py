...
class TestLiveRegistry:
    ...
    async def test_live_registry_model_details(self):
        ...
        found_models = [m for m in common_models if any(model_name.startswith(m) for model_name in model_names)]
        
        assert len(found_models) > 0, f"Expected to find at least one common model, got: {model_names}"
