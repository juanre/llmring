...
async def test_list_files_with_purpose_filter():
    ...
    try:
        ...
        # List files with purpose filter (client-side filtering)
        files = await provider.list_files(purpose="assistant")
        assert isinstance(files, list)

        # Our file should be in the list
        found = any(f.file_id == upload_response.file_id for f in files)
        assert found, "Uploaded file should appear in filtered list"
...
async def test_get_file():
    ...
    try:
        ...
        # OpenAI returns purpose in file metadata
...
async def test_upload_file_size_limit():
    ...
    try:
        ...
        # Create a BytesIO object with content > 512MB
        # We'll use 513MB of data (minimum to trigger the error)
        ...
        assert error.max_size == 512 * 1024 * 1024
        assert "exceeds OpenAI limit" in str(error)
...
async def test_get_file_invalid_id():
    ...
    try:
        ...
        with pytest.raises(ProviderResponseError):
            await provider.get_file("file-invalid_nonexistent_id")
...
async def test_delete_file_invalid_id():
    ...
    try:
        ...
        with pytest.raises(ProviderResponseError):
            await provider.delete_file("file-invalid_nonexistent_id")
